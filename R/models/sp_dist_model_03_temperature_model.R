...
# R code - Model runs - Model 3 - Temperature model
...
runname <- "sbt_seasonal_mean_model_tt_split_2021_04_11"
...
for(i in 1:length(allspp)){  
  ...
  levs <- apply(spdata[trainindsp,c('SBT.seasonal','rugosity', 'GRAINSIZE')], 2, FUN=function(x) length(unique(x)))
  if(any(levs < 6)){
