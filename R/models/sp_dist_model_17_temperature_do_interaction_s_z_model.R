...
# R code - Model runs - Model 17 - T+O+T:O+S+Z model
...
# This model removes all other temperature variables. We only keep SBT and MI in
# addition to Grain size and rugosity
runname <- "full_model_o2_interaction_with_only_sbt_2021_11_09"
...
# in this data frame
modeldiag = data.frame(sppocean=n, npres=n, fakepres=n, npres.tr=n, npres.te=n,
                       ntot=n, thresh.kappa=n, thresh=n, thresh.trKap=n,
                       thresh.tr=n, auc=n, auc.tt=n, tss=n, tss.tt=n, tssmax=n,
                       tssmax.tt=n, acc=n, acc.tt=n, accmax=n, accmax.tt=n,
                       sens=n, sens.tt=n, spec=n, spec.tt=n, kappa=n,
                       kappa.tt=n, kappamax=n, kappamax.tt=n, rpb=n, prev.obs=n,
                       prev.pred.prev=n, prev.pred.kap=n, smear=n,
                       pred_obsMedian=n, pred_obsMean=n, r2.biomass=n,
                       r2.biomass.tt=n, r2.all=n, r2.all.tt=n,
                       r2.pres.surv_year=n, r2.pres.surv_year.kap=n,
                       r2.abun.surv_year=n, r2.predPATT.surv_year=n,
                       r2.predPATTkap.surv_year=n, r2.predTT.surv_year=n,
                       dev.pres=n, dev.biomass=n, dev.pres.null=n,
                       pres.aic = n, abun.aic = n, pres.loglik = n, 
                       abun.loglik = n, dev.biomass.null=n,
                       pres.aic.tt = n, pres.loglik.tt = n,
                       abun.aic.tt = n, abun.loglik.tt = n,
...
pdf(file=paste("roms_tt_split_output/CEmodelGAMsmooths/JB_GAMs_Revised_Nov2017_GmexDrops_",runname,".pdf",sep=""),width=10,height=10)
...
for(i in 1:length(allspp)){  
  ...
  for(j in 1:length(ninds)){ 
    ...
    traininds <- c(traininds, which((spdata$regionfact==names(ninds)[j] & spdata$year<2000)))
    # for the testing set, we do the same for the last 20% of elements
    testinds <- c(testinds, which((spdata$regionfact==names(ninds)[j] & spdata$year>1999)))
    # Example
    # letters <- c('a','b','b','a')
    # example_vector <- NULL
  ...
  levs <- apply(spdata[trainindsp,c('SBT.seasonal', 'mi','rugosity', 'GRAINSIZE')], 2, FUN=function(x) length(unique(x)))
  if(any(levs < 6)){
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE))
    'Presence-Absence model - Without other parameters'
    mynullpresmod<-formula(presfit ~ s(rugosity) + s(GRAINSIZE)) #Null model w/o temp
    'Abundance model - Without other parameters'
    mynullabunmod<-formula(logwtcpue.pad ~ s(rugosity) + s(GRAINSIZE)) #Null model w/o temp
  } else {
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE))
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE))
  } else {
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal,o2_seasonal) + s(o2_seasonal) + s(zplk_class_sum_across_column) + s(salinity_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
  }
  ...
  # Write this out
  write.csv(test, file=paste("roms_tt_split_output/model_outputs/",runname,"_full_predictions.csv", sep=""))
  ...
  if(fittrain){
    ...
    write.csv(test, file=paste("roms_tt_split_output/model_outputs/",runname,"_split_predictions.csv", sep=""))
    ...
    # Just piggyback off this section and save the modeldiag$aic.tt stuff
    modeldiag$pres.aic.tt <- mygam1tt$aic
    modeldiag$pres.loglik.tt <- logLik.gam(mygam1tt)
    modeldiag$abun.aic.tt <- mygam2tt$aic
    modeldiag$abun.loglik.tt <- logLik.gam(mygam2tt)
  }
  ...
  # Set the pres.aic, pres.loglik, abun.aic, abun.loglik for the complete model
  modeldiag$pres.aic <- mygam1$aic
  modeldiag$pres.loglik <- logLik.gam(mygam1)
  modeldiag$abun.aic <- mygam2$aic
  modeldiag$abun.loglik <- logLik.gam(mygam2)
  ...
  mods = list(mygam1 = mygam1, mygam2 = mygam2, mygam1tt = mygam1tt,
              mygam2tt = mygam2tt)
  
  sp <- gsub('/', '', sp) # would mess up saving the file
  
  # Setting the 'modfolder'. We'll put another folder inside it
  modfolder <- 'roms_tt_split_output/saved_models/'
  ...
  save(modeldiag, file=paste("roms_tt_split_output/model_diagnostics/jb_modeldiag_Nov2017_GMEXdrop_", runname,".Rdata",sep=""))
  write.csv(modeldiag, file=paste("roms_tt_split_output/model_diagnostics/jb_modeldiag_Nov2017_GMEXdrop_", runname,".csv",sep=""))
  write.csv(allwarnings, file=paste('roms_tt_split_output/warnings/jb_warnings_Nov2017_GMEXdrop_', runname, '.csv', sep=''))
}
dev.off()

# BE SURE TO DROP i=260 from modeldiag and allspp
modeldiag <- modeldiag[-260,]
save(modeldiag, file=paste("roms_tt_split_output/model_diagnostics/jb_modeldiag_Nov2017_", runname,".Rdata",sep=""))
write.csv(modeldiag, file=paste("roms_tt_split_output/model_diagnostics/jb_modeldiag_Nov2017_", runname,".csv",sep=""))

modeldiag_gmex <- modeldiag
rm(modeldiag)
load(paste("roms_tt_split_output/model_diagnostics/jb_modeldiag_Nov2017_", runname,".Rdata",sep=""))
