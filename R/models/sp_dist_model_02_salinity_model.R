...
# R code - Model runs - Model 2 - Salinity model
...
runname <- "salinity_model_tt_split_2021_04_10"
...
for(i in 1:length(allspp)){  
  ...
  levs <- apply(spdata[trainindsp,c('salinity_seasonal','rugosity', 'GRAINSIZE')], 2, FUN=function(x) length(unique(x)))
  if(any(levs < 6)){
