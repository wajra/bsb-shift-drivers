...
# R code - Model runs - Model 22 - T:O+Z model
...
runname <- "to_interaction_zooplankton_2021_12_02"
...
for(i in 1:length(allspp)){  
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE))
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE) + regionfact)
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE))
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE))
  } else {
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal,o2_seasonal) + s(zplk_class_sum_across_column) + s(rugosity) + s(GRAINSIZE) + regionfact)
  }
