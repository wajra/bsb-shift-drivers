...
# R code - Model runs - Model 14 - T+O+T:O model
...
# This model removes all other temperature variables. 
runname <- "sbt_interaction_factor_o2_2021_11_08"
...
for(i in 1:length(allspp)){  
  ...
  levs <- apply(spdata[trainindsp,c('SBT.seasonal', 'salinity_seasonal','rugosity', 'GRAINSIZE')], 2, FUN=function(x) length(unique(x)))
  if(any(levs < 6)){
  ...
  if(length(levels(spdata$regionfact))==1){
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE))
    'Abundance model - If there is only one region'
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE))
    ...
    mypresmod<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmod<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
  ...
  if(length(abc[abc$reg.pres==TRUE,])==1){
    ...
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE))
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE))
  } else {
    mypresmodtt<-formula(presfit ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
    myabunmodtt<-formula(logwtcpue.pad ~ s(SBT.seasonal) + s(SBT.seasonal, o2_seasonal) + s(o2_seasonal) + s(rugosity) + s(GRAINSIZE) + regionfact)
  }
