...
# R code - Data prep - Part 03
# To transform certain parameters from ROMS-COBALT hindcast models
...
diag_data <- read_csv('data/full_haul_data_ver_11.csv')
...
filter_vars <- vars(o2_seasonal, sm_zplk_seasonal, me_zplk_seasonal, lg_zplk_seasonal, column_sm_zplk_seasonal,
                    column_me_zplk_seasonal, column_lg_zplk_seasonal, salinity_seasonal,
                    zplk_class_mean_seasonal, zplk_class_sum_across_column)
diag_data <- diag_data %>% 
...
diag_data <- diag_data %>% 
  ...
  mutate(zplk_class_sum_across_column = ifelse(zplk_class_sum_across_column<0,0,zplk_class_sum_across_column))
...
write_csv(diag_data, 'data/haul_data_for_transformation_5.csv')
